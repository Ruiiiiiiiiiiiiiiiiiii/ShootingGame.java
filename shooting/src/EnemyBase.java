...
public class EnemyBase extends Enemy{
	...
	public void move() {
		...
		if(Math.random()<0.05) {
			GameWorld.enemies.add(new RandomEnemy(x+GameWorld.stage,y,0,GameWorld.stage));
		}
		if(Math.random()<0.05) {
			GameWorld.enemies.add(new DropEnemy(x,y+GameWorld.stage,0,GameWorld.stage));
		}
		if(Math.random()<0.05) {
			GameWorld.enemies.add(new CurveEnemy(x+GameWorld.stage,y,0,GameWorld.stage));
		}
	}
